...
function M.attach_endpoint(config, opts)
	...
	-- error(..., 0) like every raise below — an assert()'s "dap.lua:N:" prefix
	-- points into this helper, not at the caller, so it adds noise, not a lead.
	if type(config) ~= "table" then
		error("attach_endpoint: config must be a table", 0)
	end
	if type(opts) ~= "table" or type(opts.label) ~= "string" then
		error("attach_endpoint: opts.label (string) is required", 0)
	end
