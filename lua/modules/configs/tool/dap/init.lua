return function()
	...
	local function mason_dap_mod()
		if mason_dap == nil then
			...
			-- setup() is owned by the plugin spec's config (tool.mason-nvim-dap):
			-- this require goes through lazy.nvim's module loader, which loads
			-- the plugin and runs that config exactly once.
